...
MODEL_NAME="TD3_velodyne_multi_v4_local_critic_geo_3"
ROS_PORT="11357"
GAZEBO_PORT="11397"
...
log_file="$LOG_DIR/train_multi_local_critic_geo_3_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached 3-agent geometry-only local-critic training process is already running with PID $old_pid"
    exit 1
  fi
fi
...
if [[ -n "$existing_pid" ]]; then
  ...
  echo "Please stop the current multi-agent training before starting 3-agent geometry-only local-critic detached mode."
  exit 1
fi
...
echo "Detached 3-agent geometry-only local-critic training started."
