...
PID_FILE="$PROJECT_ROOT/.train_multi_reward_only_3_detached.pid"
NUM_AGENTS=3
LAUNCHFILE="multi_robot_scenario_reward_only_${NUM_AGENTS}.launch"
LAUNCH_PATH="$TD3_DIR/assets/$LAUNCHFILE"
MODEL_NAME="TD3_velodyne_multi_v4_reward_only_3"
ROS_PORT="11355"
GAZEBO_PORT="11395"
...
log_file="$LOG_DIR/train_multi_reward_only_3_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached 3-agent reward-only training process is already running with PID $old_pid"
    exit 1
  fi
fi
...
if [[ -n "$existing_pid" ]]; then
  ...
  echo "Please stop the current multi-agent training before starting 3-agent reward-only detached mode."
  exit 1
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_TRAINING_VERSION='multi-agent-reward-only-3-v1'
...
echo "Detached 3-agent reward-only training started."
echo "PID: $(cat "$PID_FILE")"
echo "Model: $MODEL_NAME"
echo "Launch: $LAUNCH_PATH"
echo "Warm start: TD3_velodyne_multi_v4"
echo "Reward: visible-neighbor mean cooperative reward"
