...
PID_FILE="$PROJECT_ROOT/.train_multi_stage2_to_3a_shared_detached.pid"
NUM_AGENTS=3
LAUNCHFILE="multi_robot_scenario_stage2_to_3a_shared_${NUM_AGENTS}.launch"
LAUNCH_PATH="$TD3_DIR/assets/$LAUNCHFILE"
MODEL_NAME="TD3_velodyne_multi_v4_curriculum_stage2_to_3a_shared_from_2d_gentle"
...
log_file="$LOG_DIR/train_multi_stage2_to_3a_shared_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached stage2-to-3A shared-policy training process is already running with PID $old_pid"
    exit 1
  fi
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_EXPL_NOISE='${DRL_MULTI_EXPL_NOISE:-0.08}'
  export DRL_MULTI_EXPL_MIN='${DRL_MULTI_EXPL_MIN:-0.025}'
  export DRL_MULTI_ACTOR_LR='${DRL_MULTI_ACTOR_LR:-0.00004}'
...
echo "Detached stage2-to-3A shared-policy training started."
