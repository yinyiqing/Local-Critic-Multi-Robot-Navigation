...
PID_FILE="$PROJECT_ROOT/.train_multi_stage3_asym_three_5_context_critic_control.pid"
...
MODEL_NAME="${DRL_MULTI_TRAIN_FILE_NAME:-TD3_velodyne_multi_v4_curriculum_stage3_asym_three_5_context_critic_control_from_stage3_pair_cleanstart_v2_best}"
...
ROS_PORT="${DRL_MULTI_ROS_PORT:-12391}"
GAZEBO_PORT="${DRL_MULTI_GAZEBO_PORT:-12491}"

mkdir -p "$LOG_DIR"

timestamp="$(date +%Y%m%d_%H%M%S)"
log_file="$LOG_DIR/train_multi_${STAGE}_context_critic_control_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached context-critic control training process is already running with PID $old_pid"
    exit 1
  fi
fi
...
if [[ -n "$existing_pid" ]]; then
  ...
  echo "Please stop it before starting context-critic control validation."
  exit 1
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_USE_JOINT_ACTION_CRITIC=0
  ...
  export DRL_MULTI_MAX_EPOCHS='${DRL_MULTI_MAX_EPOCHS:-2}'
  ...
  export DRL_MULTI_TRAINING_VERSION='stage3-asym-three-5-context-critic-control-v1'
...
echo "Detached context-critic control validation training started."
...
echo "Joint-action critic: disabled"
echo "Local critic geometry only: ${DRL_MULTI_LOCAL_CRITIC_GEOMETRY_ONLY:-0}"
echo "Actor update delay steps: ${DRL_MULTI_ACTOR_UPDATE_DELAY_STEPS:-4000}"
echo "Actor LR: ${DRL_MULTI_ACTOR_LR:-0.0000008}"
echo "Critic LR: ${DRL_MULTI_CRITIC_LR:-0.00002}"
echo "Exploration noise: ${DRL_MULTI_EXPL_NOISE:-0.015}"
echo "Exploration min: ${DRL_MULTI_EXPL_MIN:-0.005}"
echo "Max epochs: ${DRL_MULTI_MAX_EPOCHS:-2}"
