...
PID_FILE="$PROJECT_ROOT/.train_multi_stage3_asym_three_5_joint_action_critic.pid"
...
log_file="$LOG_DIR/train_multi_${STAGE}_joint_action_critic_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached joint-action critic training process is already running with PID $old_pid"
    exit 1
  fi
fi
...
if [[ -n "$existing_pid" ]]; then
  ...
  echo "Please stop it before starting joint-action critic validation."
  exit 1
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_USE_JOINT_ACTION_CRITIC=1
  ...
  export DRL_MULTI_TRAINING_VERSION='stage3-asym-three-5-joint-action-critic-v1'
...
echo "Detached joint-action critic validation training started."
...
echo "Joint-action critic: enabled"
