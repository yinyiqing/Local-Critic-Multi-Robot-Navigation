...
PID_FILE="$PROJECT_ROOT/.test_multi_individual_active_probe_5_best_detached.pid"
NUM_AGENTS=5
...
log_file="$LOG_DIR/test_multi_individual_active_probe_5_best_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached 5-agent individual-active-probe best-model test process is already running with PID $old_pid"
    exit 1
  fi
fi
...
echo "Detached 5-agent individual-active-probe best-model test started."
