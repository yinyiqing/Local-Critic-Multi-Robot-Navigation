...
PID_FILE="$PROJECT_ROOT/.train_multi_stage2_to_3d2_geo_critic_from_3a_guarded_detached.pid"
NUM_AGENTS=3
LAUNCHFILE="multi_robot_scenario_local_critic_geo_${NUM_AGENTS}.launch"
LAUNCH_PATH="$TD3_DIR/assets/$LAUNCHFILE"
MODEL_NAME="TD3_velodyne_multi_v4_curriculum_stage2_to_3d2_geo_critic_from_3a_guarded"
...
ROS_PORT="${DRL_MULTI_ROS_PORT:-11379}"
GAZEBO_PORT="${DRL_MULTI_GAZEBO_PORT:-11479}"

mkdir -p "$LOG_DIR"

timestamp="$(date +%Y%m%d_%H%M%S)"
log_file="$LOG_DIR/train_multi_stage2_to_3d2_geo_critic_from_3a_guarded_detached_${timestamp}.log"

if [[ -f "$PID_FILE" ]]; then
  old_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A detached stage2-to-3D2 geometry-critic training process is already running with PID $old_pid"
    exit 1
  fi
fi
...
setsid bash -lc "
  ...
  export DRL_MULTI_LOCAL_CRITIC_ACTOR_UPDATE_DELAY_STEPS='${DRL_MULTI_LOCAL_CRITIC_ACTOR_UPDATE_DELAY_STEPS:-20000}'
  export DRL_MULTI_TRAINING_VERSION='stage2-to-3d2-geo-critic-from-3a-guarded-v1'
...
echo "Detached stage2-to-3D2 geometry-critic training started."
echo "PID: $(cat "$PID_FILE")"
echo "Agents: $NUM_AGENTS"
echo "Model: $MODEL_NAME"
echo "Actor warm start: $LOAD_MODEL_NAME"
echo "Warm start mode: actor only"
echo "Critic: geometry-only local neighborhood critic, newly initialized"
echo "Launch: $LAUNCH_PATH"
echo "Dynamic reward: 1"
echo "Distance-weighted reward: 1"
echo "Local critic: 1"
echo "Local critic geometry only: 1"
echo "Actor update delay steps: ${DRL_MULTI_LOCAL_CRITIC_ACTOR_UPDATE_DELAY_STEPS:-20000}"
...
echo "Exploration min: ${DRL_MULTI_EXPL_MIN:-0.015}"
